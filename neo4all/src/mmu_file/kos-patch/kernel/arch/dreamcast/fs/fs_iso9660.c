...
void * __kos__iso_open(vfs_handler_t * vfs, const char *fn, int mode) {
	file_t		fd;
	iso_dirent_t	*de;
...
ssize_t __kos__iso_get_first_sector(void *h){
	file_t fd = (file_t)h;
	if (fd >= MAX_ISO_FILES || fh[fd].first_extent == 0 || fh[fd].broken)
		return -1;
	return fh[fd].first_extent+150;
}

/* Read from a file */
ssize_t __kos__iso_read(void * h, void *buf, size_t bytes) {
...
off_t __kos__iso_seek(void * h, off_t offset, int whence) {
	file_t fd = (file_t)h;
...
dirent_t *__kos__iso_readdir(void * h) {
	int		c;
	iso_dirent_t	*de;
